...
describe('CatalogUpdate API', () => {
    ...
    beforeAll(async () => {
        ...
        await common.executeSqlString(`
            ...
            (
                ...
                1000002, 1,
                1000010, 1,
                '2025-12-31 23:59:59.000', 1, null,
     ...
     * Region開始終了申請取得
     */
    describe('Region開始終了申請取得', () => {
        test('正常：PXR-ROOT（承認済みの申請も取得）', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);

            // 対象APIに送信
            const response = await supertest(expressApp).get(Url.regionStatusURI + '?in_approved=true')
            ...
            expect(response.body[0].approvalActor._value).toBe(1000010);
            expect(response.body[0].approvalActor._ver).toBe(1);
            expect(response.body[0].applicantActor._value).toBe(1000002);
            expect(response.body[0].applicantActor._ver).toBe(1);
            expect(response.body[0].requestComment).toBe(null);
            expect(response.body[0].expireAt).toBe('2025-12-31T23:59:59.000+0900');
            expect(response.body[0].status).toBe(0);
            expect(response.body[0].comment).toBe(null);
            expect(response.body[0].approvar).toBe('loginid');
            expect(response.body[0].approvalAt).toBe('2020-12-31T00:00:00.000+0900');
            expect(response.body[1].id).toBe(2);
            expect(response.body[1].type).toBe(1);
            expect(response.body[1].approvalActor._value).toBe(null);
            expect(response.body[1].approvalActor._ver).toBe(null);
            expect(response.body[1].applicantActor._value).toBe(1000002);
            expect(response.body[1].applicantActor._ver).toBe(1);
            expect(response.body[1].requestComment).toBe(null);
            expect(response.body[1].expireAt).toBe(null);
            expect(response.body[1].status).toBe(1);
            expect(response.body[1].comment).toBe(null);
            expect(response.body[1].approvar).toBe('loginid');
        ...
        test('正常：PXR-ROOT（未申請のみ取得）', async () => {
            ...
            // 対象APIに送信
            const response = await supertest(expressApp).get(Url.regionStatusURI + '?in_approved=false')
                .set({ session: encodeURIComponent(Session.PXR_ROOT) });
        ...
        test('正常：in_approved未設定（未申請のみ取得）', async () => {
            ...
            // 対象APIに送信
            const response = await supertest(expressApp).get(Url.regionStatusURI)
                .set({ session: encodeURIComponent(Session.PXR_ROOT) });
        ...
        test('パラメータ異常：inApproved、boolean以外', async () => {
            ...
            // 対象APIに送信
            const response = await supertest(expressApp).get(Url.regionStatusURI + '?in_approved="a"')
                .set({ session: encodeURIComponent(Session.PXR_ROOT) });
