...
describe('CatalogUpdate API', () => {
  ...
  */
  ...
  describe('Region離脱要求' + Url.baseURIJoin, () => {
    test('Region-Rootが離脱要求を登録(下書き)情報登録なしでエラー', async () => {
      ...
      expect(response.body.message).toBe(Message.ALREADY_ABORTED);
    });
    ...
    test('異常　セッション(カタログサービス エラー応答204)', async () => {
      _operatorServer = new OperatorServer(200, 1000002);
      _catalogServer = new _StubCatalogServerOk(204);
      // 送信データを生成
      var json = {
        region: {
          code: 1000003,
    ...
    test('異常　セッション(カタログサービス エラー応答400)', async () => {
      _operatorServer = new OperatorServer(200, 1000002);
      _catalogServer = new _StubCatalogServerOk(400);
      // 送信データを生成
      var json = {
        region: {
          code: 1000003,
    ...
    test('異常　セッション(カタログサービス エラー応答500)', async () => {
      _operatorServer = new OperatorServer(200, 1000002);
      _catalogServer = new _StubCatalogServerOk(500);
      // 送信データを生成
      var json = {
        region: {
          code: 1000003,
