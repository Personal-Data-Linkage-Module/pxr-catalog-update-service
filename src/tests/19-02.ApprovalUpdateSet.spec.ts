...
export class _StubCatalogServer {
    ...
    constructor(status: number, type: number) {
        ...
        // ハンドラーのイベントリスナーを追加、アプリケーションの起動
        this._app.post('/catalog/updateSet/approval/1', _listener_approval);
...
/**
 * 変更セット承認 API のユニットテスト
 */
describe('変更セット承認 API', () => {
    /**
     * 全テスト実行前の処理
     */
    beforeAll(async () => {
    ...
    describe('変更セット承認', () => {
        test('正常：承認', async () => {
            ...
            // 対象APIに送信
            const response = await supertest(expressApp).post(Url.updateSetURI + '/approval/1')
        ...
        test('異常：アクターのコードではない', async () => {
            ...
            // レスポンスチェック
            expect(response.body.message).toBe(sprintf(Message.THIS_IS_NOT_ACTOR_CATALOG, 1));
            expect(response.status).toBe(400);
        });

        test('異常：カタログサービスからのレスポンスが200以外', async () => {
            _operatorServer = new _StubOperatorServer(200, 1000002);
            _catalogServer = new _StubCatalogServer(400, 1000011);
            _notificationServer = new _StubNotificationServer(200, 1000002);
            // 対象APIに送信
            const response = await supertest(expressApp).post(Url.updateSetURI + '/approval/1')
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
        ...
        test('異常：idが 1未満', async () => {
            ...
            _notificationServer = new _StubNotificationServer(200, 1000001);
            // 対象APIに送信
            const response = await supertest(expressApp).post(Url.updateSetURI + '/approval/0')
                .set({ accept: 'application/json', 'Content-Type': 'application/json' })
                .set({ session: encodeURIComponent(Session.PXR_ROOT) })
                .send(JSON.stringify({
                    status: 1,
