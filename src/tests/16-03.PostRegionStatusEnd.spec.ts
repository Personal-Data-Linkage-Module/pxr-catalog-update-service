...
describe('CatalogUpdate API', () => {
     ...
     */
    describe('Region終了要求', () => {
        test('正常：Region終了要求', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);
            _notificationServer = new _StubNotificationServer(200);

            // 対象APIに送信
            const response = await supertest(expressApp).post(Url.regionStatusURI + '/end')
                ...
                .send({
                    ...
                    requestComment: '終了理由です',
                    endDate: '2021-12-31T23:59:59.999+0900'
                });
            ...
            expect(response.body.requestComment).toBe('終了理由です');
            expect(response.body.endDate).toBe('2021-12-31T23:59:59.999+0900');
            expect(response.body.expireAt).toBe('2020-10-01T10:00:00.000+0900');
            expect(response.body.status).toBe(0);
            expect(response.body.approvalAt).toBe(null);
        });
        test('パラメータ不足：regionCode', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);
            _notificationServer = new _StubNotificationServer(200);
            ...
            const response = await supertest(expressApp).post(Url.regionStatusURI + '/end')
                ...
                .send({
                    requestComment: '終了理由です',
                    endDate: '2021-12-31T23:59:59.999+09:00'
                });
        ...
        test('パラメータ不足：requestComment', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);
            _notificationServer = new _StubNotificationServer(200);
            ...
            // レスポンスチェック
            expect(response.status).toBe(400);
            expect(response.body.reasons[0].property).toBe('requestComment');
            expect(response.body.reasons[0].message).toBe(Message.validation.isDefined);
        });
        test('パラメータ異常：requestComment、null', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);
            _notificationServer = new _StubNotificationServer(200);
        ...
        test('パラメータ異常：requestComment、文字列以外', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);
            _notificationServer = new _StubNotificationServer(200);
        ...
        test('パラメータ不足：endDate', async () => {
            ...
            // レスポンスチェック
            expect(response.status).toBe(400);
            expect(response.body.reasons[0].property).toBe('endDate');
            expect(response.body.reasons[0].message).toBe(Message.validation.isDefined);
        });
        test('パラメータ異常：endDate、null', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);
            _notificationServer = new _StubNotificationServer(200);
        ...
        test('異常：終了要求済Region申請が存在する', async () => {
            // スタブを起動
            _catalogServer = new _StubCatalogServer(200);
